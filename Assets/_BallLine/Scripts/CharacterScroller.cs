    ...
    public class CharacterScroller : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            for (int i = 0; i < CharacterManager.Instance.characters.Length; i++)
            {
                ...
                if (character.gameObject.transform.childCount>0)
                {
                    for(int j=0;j<character.gameObject.transform.childCount;j++)
                    character.gameObject.transform.GetChild(j).gameObject.layer= LayerMask.NameToLayer("CharacterSelectionUI");
                }
            }
        ...
        public void ShowCharacterFromCamera()
        {
            Camera selectionCamera = characterScrollerCamera.GetComponent<Camera>();
            selectionCamera.cullingMask = 512;
        }
        ...
        {
            if(currentCharacter.GetComponent<Character>().characterSequenceNumber!= CharacterManager.Instance.CurrentCharacterIndex)
            {
                int characterID = currentCharacter.GetComponent<Character>().characterSequenceNumber;
